...
$(document).ready(function(){
    $('#deleteButton').click(function(){
        ...
        var checkId = row.find('td:eq(0)').text(); // 행에서 첫 번째 열(인덱스 0)의 텍스트(체크 아이디) 얻기
        
        // AJAX를 사용하여 서버에 삭제 요청 보내기
        $.ajax({
            url: 'deletecheck.do',
            type: 'POST',
            data: { check_Id: check_Id }, // 삭제할 항목의 아이디를 서버에 보냄
            success: function(response) {
                // 서버로부터 응답이 오면 해당 행을 UI에서 제거
                row.remove();
            },
...
function checkDo(){
	window.location.href = "../admin/emplist.do";
}
/* 페이지네이션 */
